...
Nel corso dell'esecuzione di un account, la commissione totale esigibile per l'uso della memoria è proporzionale al più piccolo multiplo di 32 byte che sono richiesti in modo tale che tutti gli indici della memoria (sia per leggere o scrivere) sono incluesi nel range. Questo viene pagato in modo tempestivo; as such, referencing an area of memory at least 32 bytes greater than any previously indexed memory will certainly result in an additional memory usage fee. Due to this fee it is highly unlikely addresses will ever go above 32-bit bounds. That said, implementations must be able to manage this eventuality.
